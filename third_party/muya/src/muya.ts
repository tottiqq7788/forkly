...
export class Muya {
    ...
    static use(plugin: IMuyaPluginConstructor, options: Record<string, unknown> = {}) {
        // Deduplicate by pluginName so React StrictMode / HMR re-imports do not
        // stack identical UI plugins on the shared static list.
        const name = plugin.pluginName;
        const existing = this.plugins.findIndex(p => p.plugin.pluginName === name);
        if (existing >= 0)
            this.plugins.splice(existing, 1);
        this.plugins.push({
            plugin,
            options,
        });
    ...
    init() {
        ...
        // Prefer per-instance plugins when the host supplies them (React);
        // otherwise fall back to the static registry used by MarkText.
        const pluginList = (this.options.plugins && this.options.plugins.length > 0)
            ? this.options.plugins as IPlugin[]
            : Muya.plugins;
        if (pluginList.length) {
            for (const { plugin: Plugin, options: opts } of pluginList) {

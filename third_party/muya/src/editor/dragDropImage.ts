...
async function persistDroppedImage(
    ...
    try {
        ...
        const { src } = getImageSrc(path, muya.options.imageSrcResolver);
        if (src)
...
function handleFileImage(
    ...
    // Browser hosts rarely expose a filesystem path for dropped Files. Fall
    // back to a Data URL so `imageAction` can still persist into assets/.
    if (!path) {
        if (!muya.options.imageAction)
            return false;

        const loadingId = `loading-${getUniqueId()}`;
        insertImageParagraph(muya, target, `![${loadingId}](${name})`);
        void (async () => {
            try {
                const dataUrl = await readFileAsDataURL(image);
                await persistDroppedImage(muya, dataUrl, name, loadingId);
            }
            catch (error) {
                debug.warn(`Unexpected error on image drop: ${String(error)}`);
            }
        })();
        return true;
    }

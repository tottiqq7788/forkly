...
export interface IMuyaOptions {
    ...
    /**
     * Resolve a Markdown image `src` to a browser-loadable URL.
     *
     * Prefer this over Electron `window.DIRNAME` / `file://` anchoring when
     * embedding muya in a normal browser. Returning a non-empty string short-
     * circuits the default local-path rewrite.
     */
    imageSrcResolver?: (src: string) => string | null | undefined;
    /**
     * Per-instance UI plugins. When provided, `Muya.init` registers these
     * instead of the static `Muya.plugins` list (avoids StrictMode/HMR
     * duplicate registration when the host owns the plugin set).
     */
    plugins?: Array<{
        plugin: {
            pluginName: string;

...
- 浏览项目文件：默认进入「文件」页；目录（磁盘当前内容）与版本（最近一次保存）对照，左侧目录树、右侧预览；目录视图支持通过右键菜单新建、重命名、删除空文件夹/文件、复制路径和在访达中显示，版本视图保持只读。文件树底部恒定提供「预览 / 源码」切换：任一可读文本均可按 Markdown 安全预览（不依赖扩展名），显示模式会在切换文件时保留；图片与二进制等不可预览内容仅禁用按钮。
- Markdown 安全预览与独立编辑：工作区可编辑的 Markdown 通过文件行悬停「编辑」在浏览器新标签打开全屏编辑页（目录、所见即所得编辑区、分类格式工具栏）；左侧标题目录底部可切换「预览 / 源码」（源码为可编辑 CodeMirror，右侧格式工具栏在源码模式下同样可用于插入标题、列表、粗体等 Markdown 语法）；目录支持折叠与右键菜单（定位、复制标题/安全锚点/Markdown 链接、复制大纲）；自动保存与冲突保护；版本（HEAD）与不可编辑文件保持预览/源码只读。支持仓库内相对图片/链接与 HTTPS 远程图片。
- 通用文本编辑：工作区普通文件均可悬停「编辑」；UTF-8 文本进入同一编辑页（非 Markdown 默认源码，可手动切到 Markdown 预览，源码模式下格式工具栏可用）；二进制或超限文件给出明确提示。图片资源上传仍仅支持 Markdown 父文档（非 Markdown 文本仅插入图片语法模板）。
- macOS 可将 Forkly 设为 Markdown 的打开方式：访达双击 `.md` 等文件时，若该文件位于某个 Git 工作树内，会自动复用或登记对应项目并进入项目编辑页；仓库外文件仍走独立本地编辑会话（不创建项目）。首次需在「打开方式」中选择 Forkly，应用不会强制改写系统默认关联。
...
- GitHub 远端同步（可选）：浏览器 OAuth（PKCE + loopback）一键连接并关联现有 `origin`；Device Flow / PAT 为降级路径。Fetch / 快进拉取 / 推送；支持从 GitHub 克隆与创建远端空仓库。令牌只进入系统钥匙串。
...
- 命令行客户端 `forklyctl`：配对后供脚本与 AI 工具调用本地 API（见 [docs/roadmap/Forkly-0.3-cli.md](docs/roadmap/Forkly-0.3-cli.md)）。
...
- 内置冲突解决器。
- 丢弃修改、reset、revert、强制推送等高风险写操作。
- SSH 远端托管、通用非 GitHub 托管、Git LFS、submodule。
- PlantUML 远端渲染（首期从快捷插入隐藏）。
- AI 功能。
- 自动更新。
- 强制改写系统默认 Markdown 打开方式（仅声明可编辑，由用户首次在「打开方式」中选择 Forkly）。

更细的裁剪边界见 [docs/roadmap/Forkly-0.1-范围.md](docs/roadmap/Forkly-0.1-范围.md) 与 [docs/roadmap/Forkly-0.2-github.md](docs/roadmap/Forkly-0.2-github.md)。
...
```text
cmd/forkly/                 Go 程序入口
cmd/forkly-askpass/         Git HTTPS AskPass 助手（读取钥匙串 / 父进程注入的短时令牌）
cmd/forklyctl/              命令行 / Agent 客户端入口
internal/app/               应用生命周期、菜单栏、后台 Fetch 和启动编排
internal/localapi/          本地 HTTP API
internal/session/           本地浏览器会话和 CSRF
internal/gitexec/           Git 参数化执行、远端 Fetch/Pull/Push、状态、差异、提交、历史
internal/project/           项目登记、远端关联、AskPass 路径解析
internal/github/            GitHub Device Flow / PAT / REST 客户端
internal/credentials/       系统钥匙串抽象（Keychain / Credential Manager）
internal/operation/         可取消的长操作任务管理
internal/watcher/           文件变化监听
internal/config/            本地配置存储（v2：账号元数据与 RemoteLink，不含秘密）
...
构建 Go 二进制（会同时产出 `bin/forkly`、`bin/forkly-askpass` 与 `bin/forklyctl`）：

```bash
# 可选：注入 GitHub OAuth App 凭据（开发可复制 .env.example → .env.local）
# 回调 URL：http://127.0.0.1/local-api/v1/github/oauth/callback（GitHub 支持 loopback 动态端口）
FORKLY_GITHUB_CLIENT_ID=Ov23... FORKLY_GITHUB_CLIENT_SECRET=... make build
# 开发预览：make preview-api（读取 .env.local）+ make preview-web
# Release 校验：FORKLY_GITHUB_CLIENT_ID=... FORKLY_GITHUB_CLIENT_SECRET=... make verify-oauth
```

`make package-macos` / Windows 打包会把 `forkly-askpass` 与 `forklyctl` 放进应用包中，与主程序同目录。

## forklyctl（命令行客户端）

安装 Forkly 后可用 `forklyctl` 在终端或 AI 工具中操作本地项目。需先启动 Forkly 主进程并完成一次性配对：

```bash
# 检查 Forkly 是否在线、配对是否有效
forklyctl doctor

# 在 Forkly 设置中批准配对码后完成授权
forklyctl pair --preset collaborate --name "Cursor"

# 列出已登记项目
forklyctl projects list
```

机器可读输出加 `--json`。能力对照与 Agent 示例见 [docs/roadmap/Forkly-0.3-cli.md](docs/roadmap/Forkly-0.3-cli.md) 与 [docs/integrations/cli-parity.md](docs/integrations/cli-parity.md)。

GitHub 远端能力与验收说明见 [docs/roadmap/Forkly-0.2-github.md](docs/roadmap/Forkly-0.2-github.md)。
...
当前版本（见仓库根目录 `VERSION`）在本地 Git 客户端基础上，已纳入 `0.2` GitHub HTTPS 同步首期能力（账号连接、关联项目、Fetch / 快进 Pull / Push、克隆与创建远端空仓库、可选后台 Fetch）。细粒度边界见 [docs/roadmap/Forkly-0.2-github.md](docs/roadmap/Forkly-0.2-github.md)。
...
- 强化 GitHub 同步体验（冲突引导、多账号）与企业服务端主线保护。
- 合并申请、审查、评论和负责人合并流程。
- PlantUML 授权渲染。
- Git LFS、大文件锁和对象存储。
- 备份、恢复、审计和管理后台。

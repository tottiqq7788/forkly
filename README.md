...
当前仓库先落地 `0.1.0` 本地客户端：macOS 菜单栏常驻进程 + 浏览器本地控制台，用来验证可视化 Git 的核心体验。
...
长期产品会包含本地客户端和企业自托管服务端。`0.1.0` 只做本地客户端，不包含服务端和多人协作能力。

## 0.1.0 功能范围

`0.1.0` 是 macOS 本地可视化 Git 管理工具。
...
- 不自动执行 reset、checkout、abort 等破坏性修复。
- 不修改用户全局 Git 配置。

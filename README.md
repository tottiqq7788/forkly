...
- 浏览项目文件：默认进入「文件」页；目录（磁盘当前内容）与版本（最近一次保存）对照，左侧目录树、右侧只读预览。
- Markdown 安全预览与编辑：工作区（目录）中可编辑的 Markdown 默认进入所见即所得编辑，支持自动保存与冲突保护；版本（HEAD）与不可编辑文件保持预览/源码只读。支持仓库内相对图片/链接与 HTTPS 远程图片。

...
- GitHub 远端同步（可选）：连接 GitHub 账号（Device Flow / PAT）、关联项目仓库、Fetch / 快进拉取 / 推送；支持从 GitHub 克隆与创建远端空仓库。令牌只进入系统钥匙串。
...
```bash
# 可选：注入公开的 GitHub App Client ID（无 secret）；未设置时 Device Flow 不可用，PAT 仍可用
